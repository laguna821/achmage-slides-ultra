...
- **권고:** `FilePathTarget`에 same-directory `.partial`을 쓰고 `Mp4OutputFormat({fastStart: "reserve"})`와 exact `maximumPacketCount`를 사용한다. current/next bitmap만 유지하고 finalize 후 metadata/decode 검증 뒤 `fs.link(partial, final)`→`unlink(partial)`로 승격한다. collision은 `EEXIST`에서 suffix를 다시 선택하며 unsupported hard-link는 fail closed한다.
...
| R-005 / F-013,F-023 | collision-safe atomic output promotion | video export service/filesystem acceptance | hard-link no-overwrite, EEXIST retry, unsupported filesystem failure cleanup | provider filesystem hard-link support |
