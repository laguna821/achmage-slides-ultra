...
| Q-002 | Chrome 150 headless에서 H.264 unsupported/VP9 supported였고 실제 supported Obsidian Electron의 H.264/AAC availability와 encoder 성능은 미확인 | one-click MP4 가능 환경 | `process.versions`, `VideoEncoder.isConfigSupported`, actual-host encode+mux smoke | 사전 PoC |
...
| Q-005 | Mediabunny minimal import의 actual tree-shaken main.js/zip 증분 미측정 | Scorecard/성능 | isolated esbuild metafile+zip+load profile | 후보 평가 |
...
| Q-007 | 기본 hold/transition duration의 사용자 적합성 미검증 | 영상 리듬/길이 | R-002 UX와 연결한 3-preset usability test | UX 계획 |
| Q-008 | GIF, YouTube, external iframe, local video의 MVP 처리 정책 없음 | frame fidelity/오해 | freeze-poster/reject/phase-2 policy 결정 | 계획 |
...
| Q-012 | installed Chrome 150은 Obsidian runtime이 아님 | native HTML capture 판단 불가 | 실제 Obsidian runtime에서 probe | 사전 PoC |
...
- Remotion Web Renderer가 stable이 된 것은 4.0.491부터이고, 조사 기준 최신 4.0.504는 접근 전날 게시됐다.
...
- 이에 따라 `review_by`를 2026-09-01로 짧게 두었다. 이 날짜가 지나면 최소 SRC-005~SRC-013과 Q-001~Q-005를 refresh하기 전 계획에 사용하지 않는다.
