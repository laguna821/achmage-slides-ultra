...
- **결정 영향:** `FilePathTarget` 권고를 철회한다. 최초 `wx+` handle을 encode와 parse/decode validation이 끝날 때까지 유지하고 `StreamTarget` positional write와 `CustomSource` positional read만 사용한다. 최초 `fstat`의 dev/ino를 encode·validate·commit 경계에서 `lstat`과 비교하고, close 뒤 hard-link된 candidate도 같은 regular-file identity인지 확인한다. private partial basename은 긴 note stem을 반복하지 않는 `.achmage-video-<uuid>.partial.mp4`로 만들어 final basename이 255-character component 경계에 가까워도 중간 파일만 실패하지 않게 한다. mismatch는 성공으로 게시하지 않고 실제 경로와 상태를 보고한다. no-overwrite hard-link는 기존 final을 원자적으로 보존하지만 cleanup의 불가피한 check→unlink micro-race는 사용자 문서와 실행 잔여 위험에 과장 없이 공개한다.
