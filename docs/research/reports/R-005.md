...
- **2026-08-04 codec 결정론 교정:** official Windows Obsidian 1.13.4/Electron 43.1.1의 동일 2초 qualification을 연속 두 번 수행하자 H.264 bitstream은 달랐고, key frame의 decoded RGBA는 exact였지만 middle/last frame은 각각 2,073,600 pixels 중 256 pixels(`0.0123457%`), 464 channels가 달랐다. 최대 channel delta는 2, 전체 channel mean absolute delta는 `0.0000868056`이었다. encode·mux·parse·decode와 metadata는 모두 정상이다. W3C WebCodecs는 특정 codec/encoding method를 규정하지 않고 hardware preference도 hint이며 bitrate 변동 정도도 구현 정의로 둔다. 따라서 **physical SVG·timeline·인코더 입력 compositor pixels는 exact**, WebCodecs H.264 이후의 bitstream과 decoded repeat는 **명시적인 작은 오차 상한**으로 분리한다. 이 상한은 진단 후 정한 것이므로 exact final candidate SHA에서 owner Windows·official Windows·official macOS를 각각 한 번 새로 검증하고 실패 run을 재시도로 골라내지 않는다.
- **2026-08-05 owner release 결정:** 실제 exported video가 기능적으로 정확함을 owner가 확인했다. 1.2.0의 codec blocker는 exact pre-encode compositor와 owner Windows·official Windows·official macOS에서의 H.264 capability, encode→mux→parse→decode, 필수 metadata 성공으로 확정한다. 반복 encode의 decoded RGBA 차이는 host H.264 진단으로 계속 원본 수치와 함께 보고하지만 release blocker는 아니다. Exact `06bc097` qualification surface에서 세 환경의 core criteria는 모두 성공했으며, official Windows report의 기존 `pass:false`는 오직 superseded RGBA bound 때문이라는 사실을 그대로 보존한다.
...
| D1 visual | 같은 supported runtime → 같은 normalized physical SVG·timeline·인코더 입력 compositor RGBA | browser/font/assets/CSS/seed pin, pre-encode pixel/hash exact test | 필수·exact |
| D2 encoded semantic | codec/fps/duration/pixel format이 같고 decoded output이 명시된 미세 오차 상한 안에 있음 | encoder config/runtime 기록, parser, bounded decoded-frame diff | 필수·bounded |
...
제품 문구의 “deterministic”은 D0~D1의 exact 구조·시각 계산과 D2의 bounded codec 결과를 함께 뜻한다. raw MP4 hash 또는 lossy H.264 decoded RGBA의 전 픽셀 exact 반복을 약속하지 않는다.
...
- **결정 영향:** F-027의 실제 host 반례에 따라 이 Finding의 최초 “repeated decoded frame hash exact” 해석은 철회한다. exact AC는 physical SVG·timeline·pre-encode compositor pixels에 적용하고, H.264 이후에는 parser metadata와 bounded decoded-frame diff를 적용한다. raw MP4 SHA parity는 기본 AC에서 제외한다.
...
- **내용:** 1.1.4 release `main.js`는 5,097,070B이고 2026-08-04 통합 candidate build는 5,470,633B(SHA-256 `4ad204e22d9a9f2dd36b2684e1a025d58d5249352d7bf413495bf460d82e52ab`)로 +373,563B(+7.33%)다. isolated minified bundle에서 muted Web Renderer path는 2,356,230B, direct Mediabunny path는 169,634B였다. 통합 증분은 encoder 외 artifact normalization·validation·UI/testable control code를 포함한다. 1.1.3의 `<5,150,000B`/`<5MiB` gate는 그 release의 UTF-8 size-warning tranche 기준이므로 1.2.0에 재사용하면 승인된 video 기능을 무조건 거절하면서도 실제 loading 성능을 증명하지 못한다.
- **신뢰도:** 높음
- **반례·제약:** tree-shaking과 browser-native APIs로 실제 증분이 작을 가능성은 있다.
- **결정 영향:** exact integrated bundle·metafile, cold/warm load, memory, Scorecard scan 없이는 candidate를 채택하지 않는다. 1.2.0 build gate는 두 clean build의 byte/hash 일치와 UTF-8 유효성을 유지하고, 측정 candidate보다 129,367B(2.36%) 높은 `5,600,000B`를 unreviewed dependency/source 유입 탐지용 safety ceiling으로 사용한다. 이 상한 통과를 loading 성능 개선이나 size-warning 제거로 표현하지 않으며 R-004 actual-host 교대 측정과 Dashboard를 별도 release gate로 둔다.
...
- **결정 영향:** `FilePathTarget` 권고를 철회한다. 최초 `wx+` handle을 encode와 parse/decode validation이 끝날 때까지 유지하고 `StreamTarget` positional write와 `CustomSource` positional read만 사용한다. 최초 `fstat`의 dev/ino를 encode·validate·commit 경계에서 `lstat`과 비교한다. exclusive create 뒤 최초 `fstat`/regular-file 검증이 실패해 identity를 세우지 못하면 추측 unlink하지 않고 exact private path와 Obsidian 종료 후 inspect/remove 안내를 `VideoOutputCommitError`로 보고한다. parse/decode 전 full-file SHA-256+size와 validation 후 full-file SHA-256+size가 같을 때 transaction에 content seal을 저장하고, hard-link candidate 생성 뒤 retained handle을 다시 hash해 seal과 일치해야만 published state로 전환한다. 같은 inode·같은 length의 in-place mutation도 이 경계까지는 candidate 제거와 actionable failure로 처리하며, 마지막 hash 뒤 외부 프로세스가 published inode를 편집하는 것은 일반 final-file 외부 수정과 같은 잔여 경계로 공개한다. private partial basename은 긴 note stem을 반복하지 않는 `.achmage-video-<uuid>.partial.mp4`로 만든다. final candidate는 원래 basename이 UTF-8 byte와 UTF-16 code-unit 기준 모두 255 이하이면 보존하고, collision suffix 때문에 넘으면 stem을 줄인 뒤 full stem SHA-256 12 hex를 붙여 deterministic uniqueness와 `.slides-N.mp4` suffix를 함께 보존한다. mismatch는 성공으로 게시하지 않고 실제 경로와 상태를 보고한다. no-overwrite hard-link는 기존 final을 원자적으로 보존하지만 cleanup의 불가피한 check→unlink micro-race는 사용자 문서와 실행 잔여 위험에 과장 없이 공개한다.
...
- **내용:** 작은 PNG/JPEG/AVIF/SVG도 header에 거대한 intrinsic size나 nested data image를 선언하면 browser decode 전에 수백 MiB surface를 요구할 수 있다. APNG·animated GIF/WebP·AVIF sequence는 같은 SVG string이라도 wall clock에 따라 다른 raster frame을 내므로 static frame 결정론도 깨뜨린다. 또한 standalone SVG data URI는 원본 UTF-16 string, UTF-8 bytes, binary string, base64 string이 한동안 겹치므로 최종 문자열 길이와 압축 asset byte를 별도로 제한해야 한다. local vault file은 `TFile.stat.size`를 먼저 확인할 수 있고 blob fetch는 streaming byte budget을 적용할 수 있지만 Obsidian `requestUrl()`은 response를 내부 buffer에 받은 뒤 반환하며 underlying request를 AbortSignal로 즉시 중단하는 API가 없다. 일반 `img`/CSS URL을 포함한 raw `deck.html`을 먼저 overflow-audit iframe에 넣고 나중에 normalize하면 browser가 이 상한보다 먼저 network/decode를 시작하므로 순서 자체가 budget을 우회한다.
...
- **반례·제약:** admission limit는 exact heap upper-bound 증명이 아니며 합법적인 초고해상도 asset도 resize를 요구할 수 있다. 6000×4000 사진과 8K UHD는 허용하되 단일 축 8192px, 단일 image 40,000,000 pixels, unique image 합계 134,217,728 pixels, 단일 compressed resource 24MiB, 전체 compressed resource 64MiB, serialized artifact 48Mi characters를 넘으면 actionable failure로 중단한다. non-base64 data URI는 `decodeURIComponent`/UTF-8 encode 전에 percent triplet과 raw code point를 allocation 없이 scan해 decoded byte upper bound를 적용해야 한다. `requestUrl()`은 Content-Length와 반환 body를 검사하지만 선언 없는 과대 remote response가 host 내부에서 잠시 buffer되는 잔여 위험은 남는다.
- **결정 영향:** supported static PNG/JPEG/GIF/WebP/BMP/AVIF/SVG와 common MIME aliases는 decode 전에 format-aware dimension을 확인하고 nested data image/SVG도 재귀 검사한다. GIF는 logical screen뿐 아니라 유일한 image descriptor의 width/height와 left/top bounds를 교차 검증한다. AVIF는 largest `ispe`만이 아니라 모든 item/auxiliary `ispe` surface pixel을 합산한다. Nested SVG는 raw XML offset scanner가 아니라 parsed XML DOM의 `localName`/namespace와 DOM-decoded full `<style>`/inline/presentation value를 source of truth로 사용해 CDATA의 literal `</style>` 및 aliased namespace prefix를 검사한다. Nested SVG 안의 data font는 async `FontFace.load()`까지 재귀 검증하지 않는 1.2.0 경로에서 명시적으로 거절한다. ICO는 directory entry의 선언 크기와 embedded PNG/DIB decoder payload를 완전 교차 검증하지 않는 한 보수적 predecode 상한을 보장할 수 없으므로 1.2.0에서 명시적으로 거절한다. APNG, multi-frame GIF, animated WebP, `avis` AVIF sequence, conservative inspector가 없는 exotic `image/*`, malformed/ambiguous header, active nested SVG와 `<marquee>`/`<progress>`, 모든 nonempty legacy `background` attribute, authored CSS `animation*`·`transition*`·`@starting-style`(escaped/comment-obfuscated form 포함), resource token/URL argument obfuscation, quoted-string CSS `image-set()`, HTML `input[type=image]`, budget 초과는 audit/capture browser가 network·decode 또는 wall-clock transition을 시작하기 전에 fail closed한다. Fragment-only URL은 SVG paint/filter/marker/mask/stroke 등 non-fetch presentation reference와 `use`/`textPath`류 local href에서만 보존하고, `img`/`image`/`feImage` 또는 CSS background 등 document 재요청 가능 context에서는 거절한다. raw `deck.html`을 MP4 audit iframe에 넣지 않고, 각 correction pass에서 먼저 bounded normalization한 self-contained artifact만 fonts/images readiness 뒤 probe한다. header 추정치가 아닌 실제 `naturalWidth × naturalHeight`를 post-decode 누적 상한에도 반영한다. 이 정책과 remote cancellation 한계를 README/release notes에 공개한다.

### F-027 — WebCodecs H.264의 host implementation 편차 가능성을 decoded RGBA exact 실패로 오판하면 안 된다

- **판정:** exact CI 관찰 + 공식 표준 사실 + 보수적 추론
- **근거:** SRC-029,SRC-030; F-005,F-012,F-021,F-022
- **내용:** commit `35ef82eaf4eface84e9c8bed3f930b617c4138fa`의 official Windows qualification은 Obsidian 1.13.4, Electron 43.1.1, Chrome 150.0.7871.114에서 exact `mediabunny@1.52.3`, AVC `avc1.640028`, 1920×1080, 30fps, 60 frames/2 seconds, 8Mbps VBR, key interval 60을 사용했다. capability는 true였고 encode(394.4ms)→MP4 mux→parse→decode가 성공했으며 60 packets, one key packet, audio 0을 확인했다. 첫 MP4는 33,869 bytes/SHA-256 `c63e308e...ebe1b`, 반복 MP4는 33,682 bytes/`9328ad35...39a`로 raw bitstream이 달랐다. decoded first/key frame은 두 run에서 exact 일치했다. middle과 last는 각각 2,073,600 pixels 중 256(`0.0001234567901234568`, 즉 `0.012345679%`)만 달랐고 changed channels 464, maximum absolute channel delta 2, all-channel mean absolute delta `0.00008680555555555556`였다. W3C WebCodecs는 특정 codec이나 encode/decode method를 요구하지 않고 underlying codec implementation을 User Agent에 맡긴다. `hardwareAcceleration`은 무시할 수 있는 hint이고, VBR/CBR의 정확한 bitrate fluctuation도 implementation-defined다.
- **신뢰도:** 높음(해당 exact Windows runtime·fixture와 W3C API contract), 중간(다른 GPU/driver/OS에서 같은 오차 분포가 재현된다는 일반화)
- **반례·제약:** 이 관찰은 H.264 이후 편차가 작았다는 한 번의 host evidence이지 모든 환경의 오차를 허용하는 면허가 아니다. threshold는 진단 결과를 본 뒤 정하므로 같은 run을 채택 증거로 재사용하면 post-hoc fitting이다. exact key-frame 일치도 다른 host에서 자동 보장되지 않는다. pre-encode compositor pixel 차이, encode/mux/parse/decode 실패, metadata 오류는 codec 허용 오차로 덮어서는 안 된다.
- **결정 영향:** artifact order, physical SVG, timeline math, random-access compositor RGBA는 계속 exact hash로 검증한다. WebCodecs H.264 boundary만 REC-016의 conjunctive pixel/channel bounds로 qualification한다. exact final candidate SHA에서 세 지원 runtime을 각각 한 번 새로 실행해 모두 통과해야 하며, codec 결과가 실패한 run을 버리고 성공 run만 고르는 재실행은 금지한다.

### 2026-08-05 exact `06bc097` core codec evidence

Exact commit `06bc0976728e55c2c93f3e3b971dabe670949fce`의 official workflow와 같은 305,607-byte probe bundle(SHA-256 `bead4e68558056a68311c6c87c71afa286b623bf7f74183da15e8b069715ea64`)을 owner/official 세 환경에서 사용했다. Owner report는 이 동일 bundle로 commit 직전 생성됐고, `018c690..06bc097`에서 codec probe·qualification scripts, dependency graph와 `main.js`가 바뀌지 않았으며 `06bc097`은 Scorecard type boundary만 추가했다.

| 환경 | Core result | pre-encode compositor | 필수 metadata | 첫 output | 반복 decoded RGBA 진단 |
|---|---|---|---|---|---|
| Owner Windows, Electron 37.3.1 / Chrome 138 | capability·encode·mux·parse·decode 성공 | 3 samples exact | MP4/AVC, 1920×1080, 60f/30fps/2s, packets 60, key 2, audio 0; 2 runs validated | 26,609B, SHA `5801d87e...cafbf7` | first/middle/last repeat exact |
| Official Windows, Electron 43.1.1 / Chrome 150 | `qualification.ok:true`; capability·encode·mux·parse·decode 성공 | 3 samples exact | 동일, key 1, audio 0; 2 runs validated | 34,283B, SHA `e65df826...d617628` | first exact; middle 1,566 pixels(`0.0755208%`), 3,764 channels, max delta 4, mean `0.0005755691`; last 1,614 pixels(`0.0778356%`), 3,862 channels, max 6, mean `0.0006032986` |
| Official macOS arm64, Electron 43.1.1 / Chrome 150 | capability·encode·mux·parse·decode 성공 | 3 samples exact | 동일, key 2, audio 0; 2 runs validated | 47,150B, SHA `3279186c...bbe69d` | first/middle/last repeat exact |

Report SHA-256은 owner `A9845BA3F759D85F2EBA83263160A2F8F3012A60A2E18BFB37C996D8427A56C8`, official Windows `43AA9522264367F06C7C5D5E974E13DA5F44C75FACCB9621EE269066A2B7335F`, official macOS `F689C3E17BB217E6CD3825DEAABE161E9CCC62DDE349F1EAA65D329CAB184C35`다. GitHub run `30924379037`의 overall failure와 Windows report `pass:false`는 기존 REC-016 decoded bound에 의한 것이며 core pipeline failure나 metadata failure가 아니다.

### F-028 — 제품 기능 gate와 lossy decoded-repeat 진단은 분리해야 한다

- **판정:** exact evidence + 사용자 제품 결정.
- **근거:** SRC-031, SRC-032; F-012,F-027.
- **내용:** 세 환경 모두 exact compositor와 H.264 encode/mux/parse/decode/metadata를 성공했다. official Windows의 반복 decoded RGBA는 약 0.08% 미만 pixel에서 작은 host-dependent 차이를 보였지만 실제 exported video의 기능 정확성은 owner smoke에서 확인됐다.
- **신뢰도:** core pipeline과 metadata는 높음. decoded repeat 분포의 다른 hardware 일반화는 제한적이다.
- **반례·제약:** decoded diff가 커지는 새로운 runtime은 진단에 드러나지만 1.2.0의 기능 blocker로 자동 해석하지 않는다. pre-encode compositor drift, encode/mux/parse/decode 또는 metadata 실패는 계속 blocker다.
- **결정 영향:** AC-703/AC-707은 exact `06bc097` evidence의 core criteria로만 PASS할 수 있다. 기존 F-027/REC-016의 실패와 수치는 삭제하지 않고 diagnostic history로 유지한다.
...
| Q-013 | F-027의 bounded codec threshold는 Windows 진단을 본 뒤 설정됨 | post-hoc fitting과 성공 run 선택 위험 | exact final candidate SHA에서 owner Windows·official Windows·official macOS를 사전 고정한 순서로 각각 한 번 fresh run; 모든 원본 보존 | release gate |
...
- **채택 조건:** exact candidate SHA의 Scorecard, production main.js/three-asset delta, deterministic UTF-8와 `main.js <5,600,000B` safety ceiling, cold/warm load, first preview와 long render peak가 기록된다.
...
### REC-016 — exact compositor gate와 bounded H.264 qualification gate를 분리한다

- **근거 Finding:** F-005,F-012,F-021,F-027
- **권고:** exact final candidate SHA와 exact harness/dependency/config를 먼저 고정한다. artifact/physical SVG/timeline/random-access pre-encode compositor RGBA는 byte/pixel exact를 유지한다. 그 SHA에서 owner Windows Electron 37, official Windows Electron 43, official macOS arm64 Obsidian을 각각 **한 번** fresh qualification하고, 각 host가 independently encode→mux→parse→decode와 MP4/H.264/1920×1080/30fps/60-frame/2-second/audio-0/key-packet metadata를 통과해야 한다. 두 run의 first/key decoded RGBA는 exact여야 한다. middle과 last 각각은 changed pixels ratio `≤0.0002`(`≤0.02%`; 1920×1080에서 최대 414 pixels), changed channels `≤1024`, maximum absolute channel delta `≤2`, all-channel mean absolute delta `≤0.00015`를 **모두** 만족해야 한다.
- **상한 근거:** observed Windows 값 대비 pixel cap은 약 1.62×, channel cap은 약 2.21×, mean cap은 약 1.73× 여유만 두고 maximum delta 2와 key exact는 늘리지 않는다. 네 조건을 conjunctive로 적용하므로 넓게 퍼진 변화, 많은 channel 변화, 큰 국소 변화 중 하나라도 차단한다. 이는 perceptual “대충 같음”이나 SSIM 대체가 아니라 exact 1920×1080 qualification fixture에만 적용하는 좁은 lossy-codec envelope다.
- **채택 조건:** 세 host의 최초 fresh result와 raw report/artifact를 성공·실패와 무관하게 보존하고 모두 bound 안에 있다. infrastructure failure만 codec result가 생성되기 전에 한해 원인과 최초 log를 보존한 뒤 재실행할 수 있으며, codec/decoded-bound failure는 재시도로 덮지 않는다.
- **기각/롤백 조건:** 하나라도 encode/mux/parse/decode/metadata/key exact/bound를 실패하거나 exact final SHA가 달라지면 release를 중단하고 R-005와 계획을 다시 개정한다. Electron/Chromium 또는 encoder config가 바뀐 후의 지원 주장은 새 qualification 없이 승계하지 않는다.

**2026-08-05 상태:** 위 bounded decoded-repeat 항목은 당시의 보수적 release 제안과 실패 evidence로 보존한다. 1.2.0의 현재 release blocker는 REC-017이 대체하며, 같은 RGBA 수치는 diagnostic으로 계속 보고한다.

### REC-017 — exact compositor와 세 환경의 core H.264 기능 성공을 release gate로 사용한다

- **근거 Finding:** F-012,F-021,F-022,F-027,F-028.
- **권고:** exact `06bc097` qualification evidence에서 pre-encode compositor samples가 exact이고 owner/official Windows/macOS가 capability, H.264 encode, MP4 mux/parse, representative decode와 required metadata를 모두 성공하면 codec gate를 PASS한다.
- **채택 조건:** report/probe hashes와 모든 실패·diagnostic 값을 보존하고, `qualification.ok`, two-run metadata validation, MP4/AVC/1920×1080/30fps/60-frame/2-second/audio-0 및 decoded sample 존재를 확인한다.
- **기각/롤백 조건:** compositor mismatch, capability false, encode/mux/parse/decode 오류, metadata 불일치가 하나라도 있으면 release를 중단한다. 반복 decoded RGBA diff와 raw MP4 hash는 관측·보고하되 단독 blocker로 쓰지 않는다.
...
| R-005 / F-012,F-027 | exact compositor와 bounded lossy-codec 결과 분리 | codec qualification harness/release evidence | exact final SHA의 three-host one-shot fresh qualification, conjunctive RGBA bounds | host GPU/driver variance, post-hoc threshold |
| R-005 / F-028,REC-017 | exact `06bc097` compositor와 owner/official Win/macOS core H.264 기능 성공을 release gate로 채택; decoded repeat diff는 diagnostic | P-007/E-007 codec evidence | report/probe hash, core encode→mux→parse→decode/metadata 확인, Windows diagnostic 보존 | 향후 runtime 변경 시 새 qualification 필요 |
...
6. **VID-AC-006:** 동일 artifact의 physical SVG·timeline·pre-encode compositor RGBA는 exact 일치한다. exact candidate의 owner Windows, official Windows, official macOS qualification에서 H.264 encode→MP4 mux→parse→decode와 필수 metadata가 모두 성공한다. raw MP4와 반복 decoded RGBA 차이는 원본 수치와 함께 diagnostic으로 보존하되 release blocker로 사용하지 않는다.
...
| VID-AC-005, VID-AC-006, VID-AC-007 | F-005,F-012,F-016,F-027 | REC-004,REC-005,REC-016 |
...
| SRC-029 | local/exact CI diagnostic | `build/e007/codec-ci-35ef82e-windows/2026-08-04T15-11-36-624Z-win32-3720/report.json`; report SHA-256 `0819C1ECF9EA9380F3A347D0FA2F55AD1FB83E789E87CCD81C1F0758FCDF8142`; harness commit `35ef82eaf4eface84e9c8bed3f930b617c4138fa` | Obsidian 1.13.4, Electron 43.1.1, Chrome 150.0.7871.114, Mediabunny 1.52.3, Windows GitHub runner | generated 2026-08-04T15:11:37Z | 2026-08-04 | two-run bitstream sizes/hashes, exact key frame, middle/last 256-pixel bounded RGBA drift, encode/mux/parse/decode success | 높음(해당 exact host/fixture); build ignored이므로 핵심 수치는 F-027에 보존 |
| SRC-030 | web/official standard | [W3C WebCodecs Working Draft](https://www.w3.org/TR/webcodecs/) Abstract, §2.3, §7.8–7.9 | Working Draft 8 July 2026 | published 2026-07-08 | 2026-08-04 | 특정 codec/method 비규정, User Agent underlying implementation, hardware preference hint, bitrate fluctuation implementation-defined | 높음(API contract); Working Draft이며 host별 수치 일반화에는 중간 |
| SRC-031 | local/exact qualification + GitHub Actions | owner `build/video-codec-qualification/owner-rec016-local/.../report.json`; official Win/macOS artifacts from [run 30924379037](https://github.com/laguna821/achmage-slides-ultra/actions/runs/30924379037) | exact `06bc097` qualification surface; probe SHA `bead4e...ea64`; report SHA owner `A9845B...56C8`, Win `43AA95...335F`, macOS `F689C3...4C35` | 2026-08-05 | 2026-08-05 | three-host compositor/core codec/metadata와 Windows decoded diagnostics | 높음(해당 exact runtime·fixture) |
| SRC-032 | user/product decision | 2026-08-05 owner smoke: exported video 기능 정확성 확인; decoded-repeat diff를 diagnostic으로, exact compositor+core codec success를 release blocker로 승인 | 1.2.0 release decision | 2026-08-05 | 2026-08-05 | AC-703/AC-707 판정 경계 | 높음(제품 의사결정) |
...
- WebCodecs의 underlying H.264 implementation은 Electron/Chromium, OS, GPU와 driver에 따라 달라질 수 있으므로 REC-016 qualification은 runtime 갱신 뒤 승계하지 않는다.
...
| 2026-08-04 official Windows codec diagnostic | report SHA `0819C1EC...F8142`에서 encode→mux→parse→decode와 key RGBA exact는 PASS; middle/last는 각 256/2,073,600 pixels, 464 channels, max delta 2, mean delta `0.0000868056`; 기존 all-frame exact gate는 FAIL | PASS — 성공과 반례를 모두 F-027에 보존 |
| 2026-08-05 exact `06bc097` core qualification reconciliation | owner/official Win/macOS 모두 pre-encode exact와 capability·encode·mux·parse·decode·metadata 성공. Windows old bounded report는 middle 1,566px/3,764ch/max4/mean `0.0005755691`, last 1,614px/3,862ch/max6/mean `0.0006032986`으로 FAIL | PASS — old failure를 보존하고 owner 승인에 따라 decoded repeat는 diagnostic, core codec은 release PASS 근거 |
| W3C WebCodecs primary source | Abstract의 codec/method 비규정, §2.3의 User Agent underlying implementation, §7.8의 implementation-defined bitrate fluctuation, §7.9의 hardware hint를 원문 재확인 | PASS — exact codec output을 API가 보장한다는 해석 기각 |
...
| 2026-08-04 | private partial을 note-stem-independent UUID basename으로 고정 | final filename은 허용되지만 stem을 다시 포함한 partial만 filesystem 255-character component limit을 넘는 AC-709 반례 교정 | Codex |
| 2026-08-04 | validation 전후 및 hard-link 뒤 full-file SHA-256+size seal과 suffix-aware 255-component candidate bounding을 F-025에 추가 | same-inode same-length mutation이 identity/size 검사를 통과하고 exact-255 first candidate의 `-2`가 257자가 되는 AC-709 반례 | Codex |
| 2026-08-04 | exclusive partial create 뒤 initial stat failure는 exact path를 보고하고 추측 cleanup하지 않는 경계 추가 | identity 수립 전 raw stat error가 UUID partial을 남기면서 경로를 숨기는 lifecycle 반례 | Codex |
...
| 2026-08-04 | raw deck audit보다 bounded normalization을 선행하고 모든 legacy `background` attribute와 GIF descriptor bounds를 검사하도록 F-026을 보강 | raw remote/image URL의 pre-budget iframe load, `tbody`/`tr`/`col` Chromium autoload, 1×1 logical-screen/oversized descriptor GIF 반례 교정 | Codex |
| 2026-08-04 | non-base64 data URI의 allocation-free UTF-8 upper bound, AVIF 전체 `ispe` 합산과 nested data font 거절을 F-026에 추가 | decode 전 24MiB 상한, auxiliary surface 누적, 재귀 font readiness를 우회하는 검토 반례 교정 | Codex |
| 2026-08-04 | nested SVG의 DOM/namespace 기반 resource 검사와 fetch-capable fragment URL 거절을 F-026에 추가 | CDATA literal `</style>`, aliased XLink prefix, `img src="#x"`/CSS background fragment가 raw scanner 또는 self-contained gate를 우회하는 Chromium 반례 | Codex |
| 2026-08-04 | CSS `transition*`와 `@starting-style`을 deterministic capture 거절 범위에 추가 | insertion 직후 opacity가 0→0.396으로 변한 Chromium wall-clock 재현; 64ms final probe와 XML raster state 불일치 반례 | Codex |
| 2026-08-04 | official Windows Electron 43 exact repeat 실패를 F-027/SRC-029로 보존하고 W3C WebCodecs 원문 SRC-030, REC-016의 bounded codec gate, revised D1/D2·VID-AC-006을 추가 | deterministic physical SVG·timeline·pre-encode pixels와 User Agent H.264 bitstream/decoded repeat tolerance를 구분하고 post-diagnostic threshold의 cherry-picking을 막기 위함 | Codex |
| 2026-08-05 | exact `06bc097` 세 환경 core codec evidence·hash와 Windows 반복 RGBA diagnostics를 추가하고 F-028/REC-017로 release blocker를 exact compositor+encode/mux/parse/decode/metadata로 확정 | owner가 실제 exported video의 기능 정확성을 확인했으며 lossy host repeatability를 기능 실패와 분리하기로 승인 | Codex |
