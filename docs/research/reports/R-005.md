...
- **결정 영향:** supported PNG/APNG/JPEG/GIF/WebP/BMP/ICO/AVIF/SVG와 common MIME aliases는 decode 전에 format-aware dimension을 확인하고 nested data image/SVG도 재귀 검사한다. conservative inspector가 없는 exotic `image/*`, malformed/ambiguous header, active nested SVG, CSS resource obfuscation, budget 초과는 browser decode 전에 fail closed한다. final capture는 `document.fonts.ready`와 모든 image decode 뒤 overflow를 probe하며 이 정책과 remote cancellation 한계를 README/release notes에 공개한다.
