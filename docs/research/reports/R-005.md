...
### F-021 — 현재 owner 환경은 최신 app payload와 오래된 Electron installer가 공존한다

- **판정:** 로컬 관찰
- **근거:** SRC-018,SRC-022
- **내용:** 실행 중인 `C:\\Program Files\\Obsidian\\Obsidian.exe`는 File/Product version 1.9.12이고 binary string은 Electron 37.3.1/Chrome 138.0.7204.235다. 동시에 `%APPDATA%\\obsidian\\obsidian-1.13.4.asar`가 로드되며 1.13.4는 조사일 공식 latest release다.
- **신뢰도:** 높음(해당 Windows 환경)
- **반례·제약:** app/installer 조합과 hardware encoder는 다른 사용자마다 다르다.
- **결정 영향:** `manifest.minAppVersion`만으로 codec 지원을 추론하지 않고 owner Electron 37.3.1을 명시적인 release gate에 포함한다.
...
- output은 verified target의 `.partial`에 쓰고 성공한 codec/metadata validation 뒤 final path로 원자적으로 교체한다.
...
- **근거 Finding:** F-011
- **권고:** local vault/app URLs와 remote resources를 capture-safe source로 통일하고 `fonts.ready`, image decode, backgrounds, optional media readiness 후 exact document를 probe한다.
- **채택 조건:** offline corpus와 missing-asset failure UX가 통과한다.
...
### REC-015 — normal MP4와 bounded memory를 `reserve` mode로 함께 보장한다

- **근거 Finding:** F-012,F-013,F-019
- **권고:** `FilePathTarget`에 `.partial`을 쓰고 `Mp4OutputFormat({fastStart: "reserve"})`와 exact `maximumPacketCount`를 사용한다. current/next bitmap만 유지하고 finalize 후 metadata/decode 검증 뒤 final 이름으로 rename한다.
- **채택 조건:** 100/500-slide stress와 cancel/error cleanup, collision path가 통과한다.
...
| SRC-022 | web/official release | [Obsidian releases v1.13.4](https://github.com/obsidianmd/obsidian-releases/releases/tag/v1.13.4), [1.13.4 changelog](https://obsidian.md/changelog/2026-07-27-desktop-v1.13.4/) | latest public 1.13.4 | published 2026-07-30 | 2026-08-04 | current app version baseline | 높음; release 시 재확인 |
