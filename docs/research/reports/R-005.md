...
6. **VID-AC-006:** 동일 artifact의 physical SVG·timeline·pre-encode compositor RGBA는 exact 일치한다. exact final candidate SHA의 owner Windows, official Windows, official macOS fresh one-shot qualification에서 first/key decoded RGBA는 exact이고 middle/last 각각은 changed pixels `≤0.02%`(1920×1080에서 최대 414), changed channels `≤1024`, max absolute channel delta `≤2`, all-channel mean absolute delta `≤0.00015`를 모두 만족한다. raw MP4 SHA equality는 요구하지 않는다.
